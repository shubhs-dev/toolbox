...
Usage:
    ...
    compressvid -O                       # process existing then exit
...
def process_video(video, output_dir, preset_file, preset_label, log,
    ...
    # Start the per-video elapsed timer now
    ...
    if not wait_stable(video):
        ...
        console.print(f"  [yellow]⊘[/] {vname}  [dim]skipped (still changing)[/]")
    ...
    if final.exists():
        progress.update(task_id, completed=100, visible=False)
        console.print(f"  [yellow]⊘[/] {vname}  [dim]skipped (output exists)[/]")
        with _log_lock:
    ...
    try:
        ...
        progress.update(task_id, completed=100, visible=False)
        console.print(f"  [red]✗[/] {vname}  [dim]could not rename source[/]")
    ...
    if not ok or not tmp.exists():
        ...
        progress.update(task_id, completed=100, visible=False)
        console.print(f"  [red]✗[/] {vname}  [dim]transcode failed[/]")
...
def show_history(log):
    ...
    # Compute totals and per-status counts regardless of display mode
    total_orig = 0
    total_comp = 0
    counts = {"compressed": 0, "original_kept": 0, "failed": 0}
    ...
    # Compact summary when history is large
    if len(history) > MAX_HISTORY_ROWS:
        parts = []
        if counts["compressed"]:
            parts.append(f"[green]{counts['compressed']} compressed[/]")
        if counts["original_kept"]:
            parts.append(f"[yellow]{counts['original_kept']} kept original[/]")
        if counts["failed"]:
        ...
        if total_orig > 0:
            net = (1 - total_comp / total_orig) * 100
            size_str = (
                f"  [dim]({fmt_size(total_orig)} → {fmt_size(total_comp)}, "
                f"{net:+.1f}%)[/]"
            )
        console.print(
            f"[bold dim]Previously processed:[/] {len(history)} files "
            f"({status_str}){size_str}\n"
        )
        return

    # Full table for a manageable number of entries
    tbl = Table(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "-p", "--preset", default="hw-1080",
        help="Preset filename stem (default: hw-1080)",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "-t", "--interval", type=int, default=10,
        help="Seconds between folder scans (default: 10)",
    )
    ap.add_argument(
        "-O", "--once", action="store_true",
    ...
    ap.add_argument(
        "-P", "--presets-dir",
        help="Override the bundled presets directory with a custom one",
    )
    ap.add_argument(
        "-f", "--copy-original", action="store_true",
        help="Copy the original file to the output folder when it is smaller than the compressed version",
    )
    return ap
...
def main():
    ...
    if not watch_dir.is_dir():
        die(f"Not a directory: {watch_dir}")
    ...
    try:
        ...
        while True:
            ...
            with progress_bar() as progress:
                ...
                # Overall counter — always visible at the top
                overall_tid = progress.add_task(
                    f"[bold]Overall[/]  [dim]0/{len(videos)}[/]",
                    total=len(videos), completed=0,
                )

                # Per-file tasks start hidden; shown only while active
                # start=False so the elapsed timer begins only when
                # the video actually starts processing.
                tasks = {}
                for v in videos:
                ...
                with ThreadPoolExecutor(max_workers=args.workers) as pool:
                    futs = {
                        pool.submit(
                            ...
                            progress, tasks[v], args.copy_original,
                        ): v
                        for v in videos
                    }
                    for fut in as_completed(futs):
                        ...
                        try:
                            ...
                            progress.update(
                                tasks[v], completed=100, visible=False,
                            )
                            console.print(
                                f"  [red]✗[/] {v.name}  [dim]{exc}[/]"
                            )
                            results.append(dict(
                        ...
                        completed_count += 1
                        progress.update(
                            overall_tid, completed=completed_count,
                            description=(
                                f"[bold]Overall[/]  "
                                f"[dim]{completed_count}/{len(videos)}[/]"
                            ),
                        )
